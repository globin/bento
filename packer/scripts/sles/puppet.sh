...
if ! zypper lr -u | grep mayflower-puppet > /dev/null; then
  ...
  sudo zypper addrepo -f --no-gpgcheck http://download.opensuse.org/repositories/home:/fadenb:/branches:/systemsmanagement:/puppet/SLE_11_SP3/ fadenb-puppet
fi
echo "[bootstrap] Installing git-core" && \
  sudo zypper --no-gpg-checks in -y --force-resolution git-core
echo "[bootstrap] Installing puppet" && \
  sudo zypper --no-gpg-checks in -y --force-resolution puppet
echo "[bootstrap] Installing rubygems (prep for next step)" && \
  sudo zypper --no-gpg-checks in -y --force-resolution rubygems
